...
namespace StackExchange.Redis.Branch.Repository
{
    public interface IRedisRepository<T> where T : RedisEntity, new()
    {
        ...
        Task<List<T>> GetByBranchAsync(string branchId, params string[] groups);
        Task<List<T>> GetBySortedBranchAsync(string branchId, params string[] groups);
        Task<List<T>> GetBySortedBranchAsync(string branchId, long from, params string[] groups);
        Task<List<T>> GetBySortedBranchAsync(string branchId, long from, long to, params string[] groups);
        Task<List<T>> GetBySortedBranchAsync(string branchId, long from, long to, long skip, long take, params string[] groups);
